    ...
    *,
    ...
    for field, actual, expected in checks:
        assert actual == expected, (
            f"{source!r}: decoded.{field} == 0x{expected:X}; got 0x{actual:X}"
        )


@cocotb.test()
async def assembled_integer_and_immediate_forms_decode_correctly(dut: Dut) -> None:
    await _expect_decoded(
        dut, "ADD R4, R2, R3", op_primary=0, dst=4, op_secondary=1, src_a=2, src_b=3
    )
    await _expect_decoded(
        dut, "NOT R4, R3", op_primary=0, dst=4, op_secondary=6, src_a=0, src_b=3
    )
    await _expect_decoded(
        dut, "SGE R4, R2, R3", op_primary=1, dst=4, op_secondary=0, src_a=2, src_b=3
    )
    await _expect_decoded(
        dut,
        "SH R2, R2, -1",
...
async def assembled_memory_and_stack_forms_decode_correctly(dut: Dut) -> None:
    ...
    await _expect_decoded(
        dut, "PUSH R2", op_primary=12, dst=2, op_secondary=1, src_a=0, src_b=2
    )
    await _expect_decoded(
        dut, "POP R3", op_primary=12, dst=3, op_secondary=2, src_a=0, src_b=3
    )
...
async def assembled_long_and_register_jumps_decode_correctly(dut: Dut) -> None:
    ...
    await _expect_decoded(
        dut, "JMPR R2", op_primary=13, dst=2, op_secondary=1, src_a=0, src_b=2
    )
    await _expect_decoded(
        dut, "JALR R5, R2", op_primary=13, dst=5, op_secondary=2, src_a=2, src_b=5
    )
...
def test_assembled_integer_and_immediate_forms_decode_correctly(
    assembler_decoder_runner: Runner,
) -> None:
    _run_cocotb_test(
        assembler_decoder_runner,
        "assembled_integer_and_immediate_forms_decode_correctly",
    )


def test_assembled_memory_and_stack_forms_decode_correctly(
    assembler_decoder_runner: Runner,
) -> None:
    _run_cocotb_test(
        assembler_decoder_runner, "assembled_memory_and_stack_forms_decode_correctly"
    )


def test_assembled_long_and_register_jumps_decode_correctly(
    assembler_decoder_runner: Runner,
) -> None:
    _run_cocotb_test(
        assembler_decoder_runner, "assembled_long_and_register_jumps_decode_correctly"
    )


def test_assembled_label_target_reaches_the_rtl_decoder(
    assembler_decoder_runner: Runner,
) -> None:
    _run_cocotb_test(
        assembler_decoder_runner, "assembled_label_target_reaches_the_rtl_decoder"
    )
